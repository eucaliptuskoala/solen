...
## 2026-08-07 — FYP double-fetch fix + package rename
- **Double-fetch removed** from the FYP path. `RecommendationService.findPublicCheckIns` fetched `practiceRepository.findByCreatorId(userId)` for routing, then `PracticeBasedRecommendation` called the same query again to derive category IDs. Now `IRecommendationStrategy.findPublicCheckIns(Long userId, List<Long> categoryIds)`; the service fetches once, derives `distinct` non-null category IDs, and passes them in. `PracticeBasedRecommendation` dropped its `IPracticeRepository` (one-liner delegating to the repo); `DefaultRecommendationStrategy` accepts but ignores `categoryIds`.
  - **Why**: two DB queries where one suffices; also the strategy interface now receives the data it actually needs. Category derivation + dedup + null-filter logic moved into the service and is covered by `RecommendationServiceTest`.
  - **Behavior preserved**: routing still keys on `userPractices.isEmpty()`, NOT on `categoryIds.isEmpty()` — a user with only uncategorized practices still gets the personalized path (empty IDs → empty FYP), identical to before. (Flipping that to cold-start was offered and explicitly declined by the user.)
- **Package rename to `<domain>cases`** (user chose the full scheme incl. `checkin`): `checkin` → `checkincases` (+ `checkincases/fypstrategy`), `emailusecases` → `emailcases` (+ `emailcases/emailstrategy`). Final: `usercases`, `practicecases`, `categorycases`, `emailcases`, `signincases`, `checkincases`.
  - **Why**: `emailusecases` was the odd one out and `checkin` never fit the scheme; one consistent naming convention for domain packages. Purely mechanical (`git mv` + token rewrite of package/import lines, ~40 files), no logic change; Spring component scan is root-level so no config impact. Sub-packages `fypstrategy`/`creationstrategy`/`emailstrategy` stay `*strategy`. Stale empty test dirs `habitcases`/`habitprogress` removed.
